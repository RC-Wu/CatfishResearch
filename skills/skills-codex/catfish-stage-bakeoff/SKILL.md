...
- If explicit subagent delegation is allowed for the task, run competitors as separate worker or explorer children only after the stage contract is frozen.
- If delegation is not allowed, generate the candidates sequentially in one thread but keep the same artifact contract and parent-side comparison discipline.
- Do not hand the immediate parent scoring step to a child. The bakeoff can be delegated; the final official judgment cannot.
